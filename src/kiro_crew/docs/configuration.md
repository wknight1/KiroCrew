...
| `stt.model` | Which speech model the local provider downloads and runs: `tiny`, `base`, `small`, or `large-v3-turbo`. Bigger is more accurate and a longer first-time download — and without GPU acceleration the largest recognises slower than you speak, which Settings → Voice now says outright | `"base"` |
