...
- **Residual: a script cron child can read every installed app's `.app_secret`**: making a bundle script resolvable means a cron child now legitimately runs with an app bundle on `sys.path`, and nothing masks `<config_dir>/apps` from it. `.app_secret` is a bearer credential rather than a marker -- `dashboard.token_auth.validate_app_secret` exchanges it for that app's scoped token -- and the `cc` profile a script cron runs under leaves it readable, so an app-authored cron script, and equally an operator script under `crons/`, can read any installed app's credential. This is not introduced by the path fix: the same read is available on a tree without it, because `run_script_sandboxed` passes no app-related mask either way. It is recorded here because the path fix is what makes app-shipped cron scripts actually run, so the read goes from unreachable-in-practice to ordinary. Closing it is tracked separately, with the three conditions a fix has to satisfy at once.
- **Persisted bundle paths go stale if a bundle moves**: because the app bridge and the cron SDK store the RESOLVED absolute spec, and `add_job_if_absent_async` matches an existing job by name without refreshing its fields, a bundle root that changes location leaves the stored `job.script` naming the old path and the job fails `FileNotFoundError` at every fire. Recovery is to disable and re-enable the app, which re-registers the cron against the current root. Reachable only where a bundle root can move between runs, which today means the builtin/edition leg.
