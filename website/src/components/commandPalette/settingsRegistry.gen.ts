...
[
  ...
  },
  {
    ...
    "description": "When a conversation starts, the memories closest in wording to your message go into the prompt. This also sends each one's id and the first 200 characters of its text to Jev, which then says which ones to keep. It can only remove them — never add one, never reorder them — and if it is slow or unreachable they all go in as before. Passwords and keys are replaced before anything is sent. Off until you turn it on, even if the switch above is already on.",
    "tab": "developer",
    "type": "toggle",
    "occurrence": 1
  },
